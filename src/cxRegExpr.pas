...
type
  ...
  EcxRegExprError = class(Exception)
  private
...
begin
  ...
  if FLine > 0 then
  begin
    Result := Result + 'line: ' + IntToStr(FLine);
    if FChar > 0 then
      Result := Result + ', ' + 'char: ' + IntToStr(FChar);
    Result := Result + ': ';
  end;
  ...
  begin
    ...
    begin
      FErrors.Add(TcxRegExprError.Create(FLine, FChar,
        Format('scxRegExprHexNumberExpected', [AToken])));
      Exit;
    end;
  ...
  begin
    ...
    case AToken of
      ...
      begin
        ...
        FErrors.Add(TcxRegExprError.Create(FLine, FChar,
          Format('scxRegExprIllegalSymbol', [AToken])));
    end;
  end;
...
begin
  if not FCompiled then
    raise Exception.Create('scxRegExprNotCompiled');
end;
